...
public class StartUIStatic {
    ...
    public void init(Input input, Tracker tracker) {
        ...
        while (run) {
            ...
            if (select == 0) {
                StartUIStatic.createItem(input, tracker);
            } else if (select == 1) {
                StartUIStatic.showAllItems(tracker);
            } else if (select == 2) {
                StartUIStatic.editItem(input, tracker);
            } else if (select == 3) {
                StartUIStatic.deleteItem(input, tracker);
            } else if (select == 4) {
                StartUIStatic.findItemById(input, tracker);
            } else if (select == 5) {
                StartUIStatic.findItemsByName(input, tracker);
            } else if (select == 6) {
                run = false;
            }
        }
